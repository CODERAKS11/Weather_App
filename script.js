...
cities.forEach((city)=>{
    ...
    city.addEventListener('click',(e)=>{
        ...
        /* function that fetches and diplays all the
        data from the weather API(we will write it soon)*/
        fetchWeatherData();
        app.style.opacity= "0";
...
form.addEventListener('submit',(e)=>{
    ...
    if(search.value.length==0){
        ...
        // change from default city to the one written in input field
        cityInput=search.value;
        //fn that fatches and display all the data from weather api(write soon)
        fetchWeatherData();
...
//fn that returns  DAY OF week from a date , will use this fn later
function dayOfTheWeek(day,month,year){
    const weekday=["Sunday","Monday","Tuesday","Wednesday","Thrusday","Friday","Saturday"];
    return weekday[new Date(`${day}/${month}/${year}`).getDay()];
};
//fn that fetches and displays the data from weather api
function fetchWeatherData() {
    fetch(`http://api.weatherapi.com/v1/current.json?key=a0de46877f9542788ef72841243005&q=${cityInput}`)
        ...
        .then(data => {
            ...
            // Update background and button color based on weather condition
            if (code == 1000) {
